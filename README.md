...
```
   ...
   └──────────┘               │ ┌──────────────┬───────────────┐ │
       ↑                      │ │   10:44      │  速记 / 链接   │ │
   200×32 px                  │ │  周三·6月10  │   镜子 ◯       │ │
   贴顶居中                    │ └──────────────┴───────────────┘ │
                              └──────────────────────────────────┘
```

- 纯黑底（OLED #000000）+ 发丝边框 + 顶部高光，做出玻璃厚度与呼吸感
- 全程**纯单色**，强调色仅用于待办 P0–P3 色点与应用原生图标
- 顶部分段控件切换三 Tab，激活胶囊滑动 + 内容交叉淡入，记住上次所在 Tab
...
| Tab | 能力 |
|---|---|
| **首页 Home** | 实时**时钟·日期**（每秒跳秒、等宽 tabular）／**速记**（随手记，输入即存）／**快捷链接**（可增删，点击打开 URL 或本地路径）／**镜子**（圆形摄像头预览，点按才开） |
| **待办 Todo** | P0–P3 **四象限**（Eisenhower Matrix），勾选 / 删除 / 计数；新增走**连按两次回车**确认（防误触） |
| **应用 Apps** | 本机**应用启动坞**：列出 `/Applications` 真实图标，**搜索**实时过滤，**收藏**置顶到「常用」，点击真实启动 |

---

## 关键设计

- 展开尺寸 **620×464**，折叠刘海 **200×32**（对齐 MacBook Pro 14/16" 物理刘海）
...
| `notch-quicklinks` | 快捷链接 `[{id,name,target}]`，target 为 URL 或本地路径 |
| `notch-app-favorites` | 应用收藏 `[appPath, ...]` |
