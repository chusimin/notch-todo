...
```
       折叠态                          展开态（首页 980 / 待办 1080 / 剪贴板 1100 / 应用 1120 宽）
   ┌──────────┐               ┌──────────────────────────────────────────────┐
   │   刘海   │   ──点击──>   │ ◍ [ 首页 ][ 待办 ][ 剪贴板 ][ 应用 ]        ⌃ │
   └────╍─────┘               │ [全部][文字][图片][收藏]                    🗑 │
       ↑                      │ ┌────────┬────────┬────────┬────────┐        │
...
- 全程**纯单色**，强调色仅用于待办 P0–P3 色点、剪贴板三种类型色条、时钟点缀与应用原生图标
- 左上分段控件切换四 Tab，激活胶囊滑动 + 内容交叉淡入，**每个 Tab 独立窗口尺寸**平滑变形，记住上次所在 Tab
...
| Tab | 能力 |
|---|---|
| **首页 Home** | **两行 bento**：上排 实时**时钟·日期**（日期胶囊 + 琥珀分钟、每秒跳秒）／**快捷应用**（与应用 Tab 收藏同源，点击启动）／**速记**（随手记，输入即存）／**镜子**（圆形摄像头预览，点按才开）；下排整行**收藏剪贴**（列出收藏的剪贴条，点一下写回系统剪贴板秒粘） |
| **待办 Todo** | P0–P3 **四列并排**（Eisenhower Matrix），勾选 / 删除 / 计数；新增走**连按两次回车**确认（防误触） |
| **剪贴板 Clip** | 自动记录复制的**文字·链接·图片**，**横向 4 列网格**（最新在左上）；文字/链接/图片三种**克制类型色**区分，图片条卡片式缩略图；**收藏 / 删除 / 清空 / 过滤**（全部·文字·图片·收藏）；点条目**写回系统剪贴板**供 Cmd+V；复制**重复内容自动去重**（以最新一次为准）；全局快捷键 **Cmd+Shift+V** 召唤 |
...
- 展开尺寸**按 Tab 取值**：首页 980×316 / 待办 1080×300 / 剪贴板 1100×400 / 应用 1120×540（panelHeight；宽度严格有序，morphToTab 补间依赖；黑幕从屏顶垂下盖住菜单栏带，总高 = 菜单栏带 + 72px 结构高 + panelHeight）；折叠刘海 **200 × 菜单栏高（≈物理刘海高）**，去唇边、一像素不超出物理刘海，靠下沿抓握小药丸提示可点（screen-saver 级窗口不被菜单栏拦截，仍可点击展开）
- 纯单色玻璃材质：黑底 + 发丝边框 `rgba(255,255,255,0.08)` + 顶部 1px 高光
- 强调色仅三类例外：待办 P0 红 / P1 橙 / P2 黄 / P3 绿，剪贴板类型色（文字近中性 / 链接低饱和蓝青 / 图片低饱和紫，仅左 2px 色条 + 徽章），以及 app 原生彩色图标
...
│   └── app.js                    # LocalStorage 持久化 + 四 Tab 交互 + 剪贴板历史
├── docs/
