...
- 展开尺寸：按 Tab 取值 —— 首页 980 / 待办 1080 / 应用 1120 宽，窗口总高 = 刘海条高 + panelHeight(196/300/540) + 上下 padding；宽度超屏 clamp 到工作区-24；切 Tab 经 IPC `window:set-tab` 平滑变形
- 多屏锚定：模式切换 / Tab 变形 / 失焦收起一律锚定**窗口当前所在屏**（getDisplayMatching），绝不跟随光标——否则失焦瞬间刘海会瞬移到光标所在的另一块屏；仅启动 / 托盘重新居中 / 显示跟随光标屏；跨屏移动禁用动画 setBounds
