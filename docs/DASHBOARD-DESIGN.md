...
## 11. Round 4 当前规格（2026-08-06，与旧规格冲突时以本节为准）
...
- 剪贴板使用自适应等高卡片，过长文字截断；类型只用浅描边区分，不使用左侧色条或冗余类型标签。时间信息保留。
- 应用页是左右两栏：左侧收藏应用固定，右侧全部应用列表独立纵向滚动，页面本身不整体滚动。
- 展开与收起同时改变宽高，动画最终收束到屏幕顶部中心点；折叠态仍严格限制在物理刘海高度内。
- 新 Logo 用于 macOS 应用图标与任务完成提醒。
- 任务完成提醒是独立 `400 × 96` 顶部窗口，不抢焦点；悬停暂停、点击关闭，支持去重与最多 5 条队列。接收服务只监听 `127.0.0.1:43821` 的 `/notify/codex` 与 `/notify/gpt`。
